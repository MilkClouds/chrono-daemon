...
| Orocos RTT 컴포넌트 | 의미 | chrono-daemon 대응 |
|---------------------|------|-------------|
| **`RTT::TaskContext`** | 핵심 component 단위. State machine (`Init`/`PreOperational`/`Stopped`/`Running`). | `Daemon` (lifecycle 더 축소) |
| **`RTT::DataPort<T>` / `BufferPort<T>` / `EventPort<T>`** | Pub/Sub 위한 typed port. Connection policy (data, buffer, lock-free 등) 다양. | `Channel[T]` (단, chrono-daemon은 buffer 하나) |
| **`RTT::Activity`** | TaskContext 실행 정책. `PeriodicActivity` / `NonPeriodicActivity` / `SlaveActivity`. | (chrono-daemon은 `anyio.TaskGroup`에 위임) |
...
| **`RTT::Operation` / `Method` / `Command`** | 서비스 호출 RPC primitive. | (없음 — chrono-daemon은 채널 위에 사용자가 짠다) |
...
| 어필 포인트 | Orocos RTT | chrono-daemon |
|------------|------------|--------|
...
Orocos는 hard real-time controller 도메인을 정밀하게 cover한다. chrono-daemon은 그 정밀도가 필요 없는 다른 domain (Python async, eval, agents)에서 작고 쓰기 쉬운 추상을 제공한다.

## 채택한 디자인 결정

- **Component (TaskContext)가 lifecycle 단위.** chrono-daemon `Daemon`이 같은 의미. 단 Orocos는 4-state, chrono-daemon은 3-hook.
- **Port가 typed.** chrono-daemon `Channel[T]` Generic.
- **Connection policy를 사용자가 명시.** chrono-daemon `open_channel(maxsize=N)`.
...
- **다양한 Port 종류.** Orocos는 `DataPort` (latest value, 비결정적), `BufferPort` (queue, 결정적), `EventPort` 등 분리. chrono-daemon은 모두 `Channel`로 환원하고 latest-value 패턴은 [recipes/latest.py](../../../recipes.md)로 분리.
- **Activity 정책 노출.** chrono-daemon은 `anyio` task group에 위임.
- **자체 scripting language.** chrono-daemon은 Python 코드로 wiring.

## 관찰

Orocos RTT는 **real-time controller domain의 성숙한 reference**다. 20년 가까이 산업용 robotics에서 검증된 component 모델. chrono-daemon은 Orocos의 핵심 패턴 (typed port + composition unit + 명시적 connection policy)을 채택하면서, Orocos가 강조한 real-time precision과 풍부한 port semantics는 의식적으로 버렸다.
...
chrono-daemon의 use case가 Orocos의 도메인 (산업용 hard real-time)이 아니라 다른 도메인 (Python async ML eval / robotics control prototype)이라, Orocos를 직접 가져다 쓸 수는 없다. 디자인 reference로만 가치.
