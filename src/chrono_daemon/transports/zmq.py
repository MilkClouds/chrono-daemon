...
Install with ``chrono-daemon[zmq]``. This adapter uses ``pyzmq.asyncio`` and is
therefore intended for asyncio-backed chrono-daemon deployments. The core in-process
...
try:
    ...
    raise ImportError("chrono_daemon.transports.zmq requires the 'zmq' extra: pip install chrono-daemon[zmq]") from exc

from chrono_daemon import Channel, ChannelStats, ReceiveStream, SendStream
...
class ZmqConfig:
    ...
    @property
    def hwm(self) -> int:
        # ZMQ treats HWM=0 as unlimited. chrono-daemon's maxsize=0 means rendezvous,
