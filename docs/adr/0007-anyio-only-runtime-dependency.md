...
runlet's value proposition isn't "more features than asyncio"; it's "the
...
- Tracing/metrics. Tools that need them compose with runlet, not the
  other way around.
- A CLI framework. runlet is a library; there is no `runlet run`.

## Consequences

+ `pip install runlet` pulls anyio and nothing else.
+ Embedding runlet in another project is risk-free: no transitive dependency
  is going to bring its own peer-dep conflict.
+ The lockfile is tiny; `uv lock --upgrade` rarely surfaces churn.
+ "What does runlet really do?" has a small answer: read the seven src
...
- The first feature that genuinely needs a new dep will be a hard
  ...
  (`pip install runlet[zenoh]`) rather than a core dep.
