...
- **CLI / launcher.** runlet은 라이브러리다. `runlet run` 같은 명령은 없다.
...
다른 ROS-like 라이브러리들은 wall-clock에서만 동작하거나, sim-time을 가지더라도 후행 add-on이다. runlet은 그 반대다:
...
`Supervisor.signal_stop()`은 협력적 종료 신호를 보내고, `await Supervisor.stop(grace=N)`은 grace window 후 강제 cancellation까지 보장한다. `on_stop`은 정상 경로, 모든 Exception 경로 (shutdown/restart/ignore), 그리고 강제 cancel 경로에서도 shielded scope 안에서 보장 실행된다 ([ADR 0009](../adr/0009-cooperative-shutdown-and-lifecycle-guarantees.md)).

상세 설계는 [docs/concepts.md](../concepts.md) 및 [ADRs](../adr/) 참조.

## Real-Time vs Sim-Time Replay — 차별점

`SimClock` 기반 deterministic burst replay는 runlet의 핵심 차별점이다.
...
runlet의 niche는 "pure Python + anyio + SimClock burst replay first-class + 4 primitive minimal core"의 교집합이다. 이 조합을 만족하는 기존 프로젝트는 0개다 ([02_COMPETITIVE_LANDSCAPE.md](./02_COMPETITIVE_LANDSCAPE.md) 참조).

## Use Cases

다음 패턴들이 모두 runlet의 4 primitive로 자연스럽게 표현된다:

| Use case | runlet 매핑 |
|----------|------------|
| **#191의 System 2/1/0 inference pipeline** | `S2Loop`/`S1Loop`/`S0Loop` = `Daemon` 3개, per-session `TimeServer` = `Clock` 인스턴스, 각 stage 간 데이터 흐름 = `Channel`, `HarnessDispatcher.register/unregister` = `Supervisor` add/cooperative stop. `examples/reflex_dual_mock.py` 및 `examples/reflex_dual_multi_session.py`에서 실제 mock 구현. |
...
`examples/`의 두 mock pipeline은 ergonomic post-mortem까지 함께 제공한다. 어디가 깔끔했고, 어디가 boilerplate였고, 어디서 v0가 부족한지를 [examples/README.md](../../examples/README.md)에 정리했다.
...
| **trio** | Full | 모든 테스트 통과. 단, trio default scheduler가 task-spawn 순서를 randomize하여 byte-deterministic cross-run replay는 보장되지 않는다 (length/monotone time/distribution은 일치). Roadmap의 `ready_gate` recipe 후보. |
