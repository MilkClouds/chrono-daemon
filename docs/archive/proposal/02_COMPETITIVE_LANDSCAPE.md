# Competitive Landscape Survey — chrono-daemon

## 방법론

chrono-daemon과 동일한 4 axes (pure Python + anyio + SimClock burst replay first-class + 4 primitive minimal core)를 만족하는 기존 프로젝트가 존재하는지 검증하기 위해, 다단계 서베이를 수행했다.
...
4. **카테고리 분류 + 4 axes 매칭.** 각 후보를 7개 카테고리로 분류하고, chrono-daemon의 4 axes (pure Python + anyio + SimClock first-class + 4 primitive minimal) 충족 여부를 ✓/✗로 표시.
...
Drake의 systems framework는 **sim-time 측면에서는 chrono-daemon과 가장 가깝다.** 다른 axes (pure Python, 작음)는 만족하지 않는다.
...
Holoscan의 헤더 구조 (`include/holoscan/core/{operator,scheduler,clock,condition}.hpp`)는 매우 깔끔하며 chrono-daemon의 디자인 결정 (Clock plug-in, Scheduler 명시 노출)에 영향을 주었다.
...
| [`simple_env`](https://github.com/MilkClouds/simple_env) | – | Python | ✓ | ✗ | ✓ | ✓ | 저자 본인 prototype. Sync-only burst replay. **chrono-daemon의 직접 조상.** |

SimPy는 4 axes 중 3개를 만족하나 anyio async 모델이 아니며 robotics-shaped API가 아니다. `simple_env`는 모두 만족하나 sync-only (async 아님). 두 후보 모두 chrono-daemon의 niche와 정확히 겹치지 않는다.

## 카테고리별 매핑 — 누가 무엇을 만족하는가

7개 카테고리, 25+ 후보 중 chrono-daemon의 **4 axes 모두 (pure Python + anyio + SimClock first-class + 4 primitive minimal)** 를 만족하는 프로젝트는 **0개**다. 가장 가까운 3개를 비교하면:

| | dora-rs | SimPy | simple_env | chrono-daemon |
|--|---------|-------|-----------|--------|
...
이는 dora-rs의 의도된 디자인이다 — production robotics에서는 wall-clock에서만 동작하면 충분하고, replay는 별도 record/playback tool로 처리한다. chrono-daemon의 sim-replay 요구는 dora-rs의 핵심 use case가 아니다.
...
이 가치 제안은 모두 production deployment를 가정한다. chrono-daemon의 가치 제안 ("test와 production에서 같은 daemon 코드를 SimClock으로 burst-replay")은 다른 축에 있다.

### Q3. dora-rs의 in-process operator mode를 쓰면 chrono-daemon과 가까워지는가?
...
dora-rs는 chrono-daemon과 **다른 axes에 있는 프로젝트**이며 경쟁 관계가 아니다. dora-rs는 production-grade distributed robotics dataflow, chrono-daemon은 in-process test/replay + minimal daemon supervisor. 같은 4 abstraction 이름 (`Node`/`Channel`/...)을 공유하지만 의도된 use case가 다르다.

## 결론

**chrono-daemon의 4 axes (pure Python + anyio + SimClock burst replay first-class + 4 primitive minimal core)를 모두 만족하는 기존 프로젝트는 0개다.** 25+ 후보를 7 카테고리로 분류한 전수 조사로 검증되었다.

- 가장 가까운 후보 3개 (dora-rs, SimPy, `simple_env`)는 각각 1-2개 axes만 만족하며 chrono-daemon의 niche를 완전히 cover하지 않는다.
