...
chrono-daemon is four small primitives on top of `anyio`. Everything else, including
...
`Channel` is the only inter-daemon communication primitive in chrono_daemon. There is
...
daemon. `chrono_daemon.recipes.merge` covers N:1 fan-in, `load_balance` covers 1:N
...
`chrono_daemon.transports.zmq` provides optional TCP-backed endpoints installed with
`chrono-daemon[zmq]`. It preserves the `SendStream` / `ReceiveStream` call surface,
...
- `sup.signal_stop()`: sync, fire-and-forget. Sets the shared stop event
  ...
  using `chrono_daemon.recipes.cooperative_every`) exit naturally so `on_stop`
...
- Topic / pub-sub broadcast → `chrono_daemon.recipes.fanout.tee` (see docs/recipes.md).
...
If you want one of these, check `docs/recipes.md` and the relevant ADR first.
