...
from chrono_daemon._types import DaemonError, OnError
from chrono_daemon.clock import Clock, WallClock
from chrono_daemon.context import Context
from chrono_daemon.daemon import Daemon, _FnDaemon
...
class Supervisor:
    ...
    def __init__(
        ...
        *,
        ...
        self._logger: logging.Logger = logger or logging.getLogger("chrono_daemon")
    ...
    def add(self, daemon: Daemon, *, name: str | None = None) -> None:
        ...
        if not isinstance(daemon, Daemon):
            if callable(daemon) and getattr(daemon, "_chrono_daemon_factory", False):
