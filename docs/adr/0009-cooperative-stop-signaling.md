...
A recipe, `chrono_daemon.recipes.cooperative_every.cooperative_every(ctx, period)`,
...
+ Sync callers (e.g. signal handlers reached via
  `chrono_daemon.recipes.sync_bridge`) can use the portal to invoke either method.
