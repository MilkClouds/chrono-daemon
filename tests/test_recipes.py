...
``src/chrono_daemon/recipes/__init__.py``), but they must at minimum import
...
from chrono_daemon import Channel, EndOfStream, SendStream, SimClock, open_channel
...
async def test_recipes_namespace_imports() -> None:
    ...
    from chrono_daemon.recipes import (  # noqa: F401
        batcher,
        coordination,
        cooperative_every,
        fanout,
...
async def test_recipe_category_imports() -> None:
    from chrono_daemon.recipes.coordination import Pending, batcher_loop, cooperative_every, select, submit  # noqa: F401
    from chrono_daemon.recipes.hosting import SupervisorHost, host_async_dispatcher  # noqa: F401
    from chrono_daemon.recipes.routing import load_balance, merge, tee, worker_pool  # noqa: F401
    from chrono_daemon.recipes.state import CoalesceSend, DropNewestSend, DropOldestSend, Latest  # noqa: F401
...
async def test_supervisor_host_starts_and_stops_child_supervisor() -> None:
    from chrono_daemon import Context, Supervisor, daemon
    from chrono_daemon.recipes.supervisor_host import SupervisorHost

    log: list[str] = []

    @daemon
    async def worker(ctx: Context) -> None:
        log.append("run")
        await ctx.stop_event.wait()
        log.append("stopping")

    sup = Supervisor()
    sup.add(worker(), name="worker")
    host = SupervisorHost(sup, on_stopping=lambda: log.append("teardown"))

    async with anyio.create_task_group() as tg:
        await host.start(tg)
        assert host.snapshot()["worker"].state == "running"
        await host.stop()
        tg.cancel_scope.cancel()

    assert log == ["run", "teardown", "stopping"]


async def test_supervisor_host_surfaces_startup_failure() -> None:
    from chrono_daemon import Context, Daemon, Supervisor
    from chrono_daemon.recipes.supervisor_host import SupervisorHost

    class _FailsOnStart(Daemon):
    ...
    sup = Supervisor()
    sup.add(_FailsOnStart(), name="bad")
    ...
    assert any("bad start" in str(exc) for exc in _flatten(excinfo.value))


async def test_supervisor_host_propagates_runtime_failure() -> None:
    from chrono_daemon import Context, Daemon, Supervisor
    from chrono_daemon.recipes.supervisor_host import SupervisorHost
    ...
    sup = Supervisor()
    sup.add(_FailsAfterStart(), name="bad")
    host = SupervisorHost(sup)
    ...
    assert any("runtime boom" in str(exc) for exc in _flatten(excinfo.value))
...
async def test_batcher_dispatches_no_delay() -> None:
    ...
    from chrono_daemon.recipes.batcher import Pending, batcher_loop
...
async def test_batcher_timeout_window_under_simclock() -> None:
    ...
    from chrono_daemon import SimClock
    from chrono_daemon.recipes.batcher import Pending, batcher_loop, submit
...
def test_sync_bridge_hosts_dispatcher_for_sync_callers() -> None:
    ...
    async def collector(ctx: object, recv: object) -> None:  # types relaxed in this closure
        from chrono_daemon import EndOfStream as EOS
    ...
    async def setup(sup):  # type: ignore[no-untyped-def]
        from chrono_daemon import open_channel as _open_channel
...
def test_sync_bridge_waits_for_daemon_startup() -> None:
    from chrono_daemon import Context, Daemon
    from chrono_daemon.recipes.sync_bridge import host_async_dispatcher
    ...
    async def setup(sup):  # type: ignore[no-untyped-def]
        sup.add(_SlowStart(), name="slow")
        return object()

    with host_async_dispatcher(setup):
        assert log == ["started"]


def test_sync_bridge_surfaces_daemon_startup_failure() -> None:
    from chrono_daemon import Context, Daemon
    from chrono_daemon.recipes.sync_bridge import host_async_dispatcher

    class _FailsOnStart(Daemon):
        async def on_start(self, ctx: Context) -> None:
            raise RuntimeError("bad bridge start")
    ...
    with pytest.raises(RuntimeError, match="async setup failed") as excinfo:
        with host_async_dispatcher(setup):
            pass

    assert excinfo.value.__cause__ is not None
    assert any("bad bridge start" in str(exc) for exc in _flatten(excinfo.value.__cause__))
