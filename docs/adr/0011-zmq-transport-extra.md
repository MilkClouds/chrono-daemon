...
Ship a first remote adapter under `chrono_daemon.transports.zmq`, installed with the
`zmq` extra:
...
- Close propagation is cooperative over control frames. Once a peer close is
  observed, later sends or receives fail with the chrono-daemon exceptions, but a
