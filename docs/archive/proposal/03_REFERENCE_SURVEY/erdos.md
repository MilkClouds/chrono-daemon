...
| ERDOS 컴포넌트 | 의미 | runlet 대응 |
|----------------|------|-------------|
| **Operator** | 처리 단위 (node). | `Daemon` |
| **Stream** | Operator 간 메시지 흐름. | `Channel[T]` |
| **Watermark** | 메시지 timestamp의 lower bound. Operator는 watermark보다 오래된 메시지를 무시. | (runlet은 in-process queue라 ordering 자동 — watermark 불필요) |
| **Driver** | Dataflow graph 정의 + 실행 entry point. | `Supervisor.__aenter__` |
| **Application** | 전체 dataflow graph. | (runlet은 코드로 동적) |
...
runlet의 `SimClock`은 메시지 ordering이 아니라 **virtual time advancement**가 첫째 의도다. 두 접근법은 직교한다.
...
ERDOS와 runlet은 deterministic semantic 추구라는 점에서 motivation을 일부 공유하지만, 결정성의 종류가 다르다.

## 채택한 디자인 결정

- **Operator = lifecycle unit.** runlet `Daemon`과 동일 의미.
- **Stream = typed message flow.** runlet `Channel[T]`.
- **Driver / Application split.** runlet은 supervisor entry point가 같은 역할.

## 거부한 디자인 결정

- **Watermark.** runlet은 in-process queue로 ordering 자동 보장 — watermark가 필요한 use case (out-of-order network message 처리)가 v0 scope 밖.
- **End-to-end deadline tracking.** ERDOS의 D3 paper는 AV pipeline의 end-to-end deadline을 dynamic하게 enforce. runlet은 deadline 추상 없음.
- **Application = static graph.** runlet은 코드로 동적 wiring (dora-rs YAML과 비슷한 이유로 거부).
...
runlet과 ERDOS는 시간 / 결정성 추상에서 다른 axes를 잡았다:

- ERDOS: "메시지 ordering이 deterministic" → cross-run reproducibility는 ordering 일치를 의미.
- runlet: "시간 자체가 deterministic하게 advance" → cross-run reproducibility는 동일 시점 동일 상태를 의미.

ERDOS 모델은 production AV에서 wall-clock으로 도는 시스템에 적합하고, runlet 모델은 sim/test에서 fast-forward하는 시스템에 적합하다. **두 모델은 합쳐질 수 있다** — sim clock + watermark — 하지만 runlet v0에서는 watermark가 의식적 no-goal (in-process queue로 자동 처리).
